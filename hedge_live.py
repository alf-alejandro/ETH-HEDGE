"""
hedge_live.py v9 — Simulación Hedge Dinámico ETH Up/Down 5m
...
async def main_loop():
    ...
    log_ev("=" * 65)
    log_ev("  HEDGE LIVE v9 — Sim con condiciones realistas — ETH")
    ...
    while True:
        try:
            ...
            # 1. Descubrir mercado
            if mkt is None:
                log_ev("Buscando mercado ETH Up/Down 5m...")
                guardar_estado()
                obi_history_up.clear()
                obi_history_dn.clear()
                ya_opero_ciclo = False
                mkt = await loop.run_in_executor(None, find_active_market, "ETH")
                if mkt:
